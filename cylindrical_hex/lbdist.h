...
public:
	...
	void run(const Board * b, bool crossvcs = true, int side = 0) {
		...
		int m = board->get_x_size();
		//int m1 = m-1;		//Is this needed?
		
		int n = board->get_y_size();
		int n1 = n-1;
		...
		for(int player = start; player <= end; player++){
			...
			for(int y = 0; y < n; y++) { init(0,    y, 1,  player, 1); } flood(1,  player, crossvcs); //edge 1
			for(int y = 0; y < n; y++) { init(n1-y, y, 2,  player, 5); } flood(2,  player, crossvcs); //edge 2
		}
	}
			...
			for(int i = 5; i <= 7; i++){
				...
				if(board->onboard(next.pos)){
					...
					if( dist(edge, player, pos) > next.dist){
						...
						if(next.dist < board->get_x_size() && next.dist < board->get_y_size())
							Q.push(next);
					}
				}
			}

...
GTPResponse GTP::gtp_boardsize(vecstr args){
	if(args.size() != 2)
		return GTPResponse(false, "Board size:  " + to_str(hist->get_x_size()) + " x " + to_str(hist->get_y_size()) +  "\n");

	int x_size = from_str<int>(args[0]);
	int y_size = from_str<int>(args[1]);
	if(x_size < Board::min_x_size || x_size > Board::max_x_size)
		return GTPResponse(false, "Size " + to_str(x_size) + " is out of range.");
	if(y_size < Board::min_y_size || y_size > Board::max_y_size)
		return GTPResponse(false, "Size " + to_str(y_size) + " is out of range.");

	hist = History(Board(x_size, y_size));
	set_board();
...
GTPResponse GTP::gtp_debug(vecstr args){
	std::string str = "\n";
	str += "Board size:  " + to_str(hist->get_x_size()) + "," + to_str(hist->get_y_size()) + "\n" ;
...
GTPResponse GTP::gtp_save_sgf(vecstr args){
	...
	sgf.x_size(hist->get_x_size());
	sgf.y_size(hist->get_y_size());
...
GTPResponse GTP::gtp_load_sgf(vecstr args){
	...
	int x_size = sgf.x_size();
	int y_size = sgf.y_size();
	
	if(x_size != hist->get_x_size()){
		if(hist.len() == 0){
			hist = History(Board(x_size, y_size));
			set_board();
			time_control.new_game();
